...
# 분석: core_analyzer.analyze()
# 판단: signal_engine.evaluate()
...
def run_entry_radar(symbol: str, candles_by_tf: dict) -> dict:
    ...
    evaluation = evaluate(sig, candles_15m)
    ...
    if sig.get("is_range") and sig.get("range_pos") == "MIDDLE":
        reason = "박스권 중앙"
    else:
        reason = "방향 후보 감지"
    ...
    return _result("ENTRY_RADAR", sig,
        make_radar_message(sig, reason), reason)
