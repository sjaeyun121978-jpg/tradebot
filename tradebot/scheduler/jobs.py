...
# ── 가상 포지션 시스템 (step_state_engine HOLD/EXIT 연동) ──────
_active_step_positions: dict = {}   # symbol → {direction, entry, stop, created_at}


def _get_active_position(symbol: str) -> dict | None:
    """현재 심볼의 가상 포지션 반환. 없으면 None."""
    return _active_step_positions.get(symbol)


def _set_active_position(symbol: str, decision: dict) -> None:
    """
    REAL 카드 발송 성공 시 가상 포지션 등록.
    EXIT 카드 발송 성공 시 포지션 제거.
    """
    step      = str(decision.get("step_type") or decision.get("final_state") or "").upper()
    direction = str(decision.get("direction") or "").upper()

    if step == "REAL" and direction in ("LONG", "SHORT"):
        risk = decision.get("risk") or {}
        _active_step_positions[symbol] = {
            "direction": direction,
            "entry":     risk.get("entry") or decision.get("current_price"),
            "stop":      risk.get("stop")  or risk.get("stop_loss"),
            "created_at": int(time.time()),
        }
        print(f"[POSITION] {symbol} 가상 포지션 등록: {direction} "
              f"entry={_active_step_positions[symbol].get('entry')} "
              f"stop={_active_step_positions[symbol].get('stop')}", flush=True)

    elif step == "EXIT":
        if symbol in _active_step_positions:
            _active_step_positions.pop(symbol, None)
            print(f"[POSITION] {symbol} 가상 포지션 제거 (EXIT)", flush=True)
...
def format_decision_message(decision: dict, base_message: str = "") -> str:
    ...
    # 방향 점수
    score_line = f"LONG {long_score:.0f}% · SHORT {short_score:.0f}% · 조건 상태 {confidence:.0f}%"
...
def run_entry_timing(symbol, candles_by_tf, structure_result=None, market_data=None,
    ...
    try:
        return entry_timing.analyze_entry_timing(
            symbol, candles_by_tf, structure_result, market_data,
...
def _extract_step_type(alert_type, decision):
    # ── 새 엔진 결과가 있으면 step_type 직접 사용 ─────────────
    _eng_decision = decision.get("_step_decision") or decision.get("_eng_decision")
    if _eng_decision:
        eng_state = str(_eng_decision.get("final_state", "")).upper()
        if eng_state in ("WAIT", "EARLY", "PRE", "REAL", "HOLD", "EXIT"):
            return eng_state

    text = str(alert_type or decision.get("mode", "")).upper()

    # EXIT / HOLD는 REAL/PRE보다 먼저 체크 (문자 포함 관계 없으므로 순서 무관)
    if "EXIT"  in text: return "EXIT"
    if "HOLD"  in text: return "HOLD"
    if "REAL"  in text: return "REAL"
    if "PRE"   in text: return "PRE"
    if "EARLY" in text: return "EARLY"
...
def _step_fingerprint(step_type, decision):
    ...
    step_state_engine 결과(final_state/score/warning/exit_type)도 포함.
    """
    engine = decision.get("_step_decision") or {}
    base = "|".join([
        str(step_type),
    ...
    # 엔진 상태 변화도 fingerprint에 포함 (HOLD/EXIT 전환 감지)
    engine_part = "%s|%s|%s|%s" % (
        str(engine.get("final_state", "")),
        str(int(float(engine.get("score", 0)) // 5 * 5)),
        str(engine.get("warning", False)),
        str(engine.get("exit_type", "NONE")),
    )
    return base + "|" + engine_part
...
def build_step_card_payload(symbol, result, decision):
    ...
    # ── step_state_engine 필드 — HOLD/EXIT 카드 렌더에 사용 ──────
    _eng = decision.get("_step_decision") or {}
    sig.update({
        "step_hold_score":      _eng.get("hold_score",       0),
        "step_exit_score":      _eng.get("exit_score",       0),
        "step_exit_type":       _eng.get("exit_type",        "NONE"),
        "step_action_text":     _eng.get("action_text",      ""),
        "step_warning":         _eng.get("warning",          False),
        "step_warning_reasons": _eng.get("warning_reasons",  []),
        "step_main_reasons":    _eng.get("main_reasons",     []),
        # ── v2 신규 품질 필드 (UI 수정 없음, 필드만 전달) ───
        "accumulation_score":   _eng.get("accumulation_score", 0),
        "distribution_score":   _eng.get("distribution_score", 0),
        "trap_risk_score":      _eng.get("trap_risk_score",    0),
        "quality_tier":         _eng.get("quality_tier",       "LOW"),
    })
...
def send_step_card_event(symbol, step_type, result, decision, candles_by_tf, market_data=None):
    """
    STEP 카드 이미지 전송 전용.
    - 렌더러: chart_renderer.render_step_card 1개만 사용
    - 전송: telegram.send_photo(img, caption=None, parse_mode=None) 고정
    - caption 변수 생성·사용 금지
    - display_step 강제: WATCH → WAIT
    """
    ...
    # display_step 강제 추가 — WATCH는 WAIT으로 변환
    payload["display_step"] = "WAIT" if str(step_type).upper() in ("WATCH", "WAIT") else str(step_type).upper()
    ...
    # caption=None 고정 — caption 변수 사용 금지
...
def run_entry_timing_if_needed(symbol, candles_by_tf, structure_result=None,
                                ...
                                current_position=None):
    """
    WAIT/PRE/REAL/EARLY/HOLD/EXIT 이벤트 전용.
    상태 변화 또는 쿨다운 만료 시 STEP 카드 이미지만 발송한다.
    current_position: 보유 포지션 dict — step_state_engine HOLD/EXIT 판단에 사용
    """
    ...
    # ── [신규] 가상 포지션 자동 연결 ──────────────────────────
    # 외부에서 current_position을 전달하지 않으면 가상 포지션에서 자동 조회
    if current_position is None:
        current_position = _get_active_position(symbol)
        if current_position:
            log(f"[POSITION] {symbol} 가상 포지션 자동 연결: {current_position.get('direction')}")

    entry_result = run_entry_timing(
        symbol, candles_by_tf, structure_result, market_data,
        current_position=current_position
    )
    if not entry_result:
    ...
    for result in results:
        ...
        # ── [신규 엔진] _step_decision 결과를 decision에 강제 반영 ──
        # 기존 판단(entry_filter/reversal)보다 엔진 결과가 우선한다.
        _eng = sig_data.get("_step_decision") if isinstance(sig_data, dict) else None
        if _eng:
            _eng_dir = _eng.get("direction", "")
            if _eng_dir not in ("", "NEUTRAL"):
                decision["direction"]           = _eng_dir
                decision["candidate_direction"] = _eng_dir
            # 점수 강제 덮어쓰기
            decision["confidence"]       = float(_eng.get("score",       decision.get("confidence",  0)))
            decision["long_score"]       = float(_eng.get("long_score",  decision.get("long_score",   0)))
            decision["short_score"]      = float(_eng.get("short_score", decision.get("short_score",  0)))
            decision["score_gap"]        = float(_eng.get("gap",         decision.get("score_gap",    0)))
            decision["step_type"]        = _eng.get("final_state", "WAIT")
            decision["warning"]          = _eng.get("warning", False)
            decision["_step_decision"]   = _eng   # _extract_step_type에서 참조
            # warning_reasons 병합 (중복 제거)
            _existing  = decision.get("warnings") or []
            _new_warns = [w for w in (_eng.get("warning_reasons") or []) if w not in _existing]
            decision["warnings"] = _existing + _new_warns
            log(f"[STEP ENGINE] {symbol} decision 강제반영: {_eng['final_state']} "
                f"dir={_eng_dir} score={_eng['score']:.0f} gap={_eng['gap']:.0f}"
                f" hold={_eng.get('hold_score',0):.0f} exit={_eng.get('exit_score',0):.0f}")
        ...
        # _step_decision이 있으면 새 엔진이 이미 확정한 결과이므로 reversal 승격/강등 스킵
        _has_engine_decision = bool(sig_data.get("_step_decision"))
        ...
        if not _has_engine_decision and not is_hard_block and not rev_block:
            # 1) WATCH 이상 감지 → WAIT를 WATCH로 상향
            if step_type == "WAIT" and rev_stage in ("WATCH", "EARLY", "PRE", "REAL") \
                    and rev_dir in ("LONG", "SHORT"):
                decision["candidate_direction"] = rev_dir
        ...
        # 5) 변곡 방향 충돌 → PRE/REAL/EARLY 차단 (새 엔진 결과가 있으면 스킵)
        if not _has_engine_decision:
        ...
        # ── [신규] 가상 포지션 갱신 (REAL → 등록 / EXIT → 제거) ──
        _set_active_position(symbol, decision)
        # UI/이미지 변경 없음 — 기존 카드 이미지 다음에 텍스트만 추가
        _eng = sig_data.get("_step_decision") if isinstance(sig_data, dict) else None
        if _eng:
            try:
                _warn_tag = "⚠ " + " / ".join(_eng["warning_reasons"][:2]) if _eng.get("warning") else ""
                _debug_lines = [
                    f"📊 STEP ENGINE  {symbol}",
                    f"상태: {_eng['final_state']}  방향: {_eng['direction']}",
                    f"점수: {_eng['score']:.0f}  LONG: {_eng['long_score']:.0f}  SHORT: {_eng['short_score']:.0f}  GAP: {_eng['gap']:.0f}",
                ]
                if _eng.get("warning"):
                    _debug_lines.append(_warn_tag)
                if _eng.get("hold_score", 0) > 0:
                    _debug_lines.append(f"HOLD: {_eng['hold_score']:.0f}")
                if _eng.get("exit_score", 0) > 0:
                    _debug_lines.append(f"EXIT: {_eng['exit_score']:.0f}  유형: {_eng['exit_type']}")
                if _eng.get("main_reasons"):
                    _debug_lines.append("✔ " + " / ".join(_eng["main_reasons"][:3]))
                if _eng.get("action_text"):
                    _debug_lines.append(_eng["action_text"])
                _debug_msg = "\n".join(_debug_lines)
                telegram.send_message(_debug_msg)
                log(f"[STEP ENGINE] {symbol} debug block 발송")
            except Exception as _de:
                log(f"[STEP ENGINE] debug block 발송 오류: {_de}")
...
def run_symbol_cycle(symbol, candles_by_tf, market_data=None, current_position=None):
    ...
    # PRE / REAL / EARLY / HOLD / EXIT 판단은 가격 변화율 필터로 차단하지 않는다.
    run_entry_timing_if_needed(
        symbol, candles_by_tf, structure_result, market_data, ms,
        current_position=current_position
    )
