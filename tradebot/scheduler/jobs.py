...
def send_step_card_event(symbol, step_type, result, decision, candles_by_tf, market_data=None):
    ...
    # caption은 plain text로만 전달 (HTML 파싱 오류 방지)
    rev_stage_cap = decision.get("reversal_stage") or ""
    rev_score_cap = decision.get("reversal_score") or 0
    caption_parts = [f"[{display_type}] {symbol}"]
    if display_type == "EARLY":
        caption_parts.append(f"방향: {cand_upper} 초기진입")
    elif display_type == "WATCH":
        caption_parts.append(f"방향 후보: {candidate_direction}")
    if rev_stage_cap in ("WATCH", "EARLY", "PRE", "REAL"):
        caption_parts.append(f"변곡{rev_stage_cap} {rev_score_cap}pt")
    caption_parts.append("자동진입 아님")
    caption = " | ".join(caption_parts)

    log(f"[STEP CARD SEND] {symbol} {step_type} img={len(img)}B caption={repr(caption[:40])}")
    ok = telegram.send_photo(img, caption=caption, parse_mode=None)
