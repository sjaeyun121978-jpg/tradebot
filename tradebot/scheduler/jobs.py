...
def should_send_step_event(symbol, step_type, decision, cooldown_sec=SAME_ALERT_COOLDOWN_SEC):
    ...
    # 같은 상태 반복은 쿨다운 이후에만 재발송
    if current - prev.get("ts", 0) >= cooldown_sec:
        _step_signal_state_store[symbol] = {"fingerprint": fp, "ts": current}
        return True

    log(f"[SKIP STEP] {symbol} {step_type} unchanged/cooldown")
    return False
...
def send_step_card_event(symbol, step_type, result, decision, candles_by_tf, market_data=None):
    ...
    if step_type == "REAL":
        img = chart_renderer.render_real_card(
            payload,
            candles_1h,
            hits=result_dict.get("hits") or result_dict.get("conditions"),
            levels=result_dict.get("levels") or decision.get("risk") or {},
        )
    elif step_type == "PRE":
        img = chart_renderer.render_pre_card(
            payload,
            candles_1h,
            hits=result_dict.get("hits") or result_dict.get("conditions"),
        )
    elif step_type == "EARLY":
        img = chart_renderer.render_early_card(payload, candles_1h)
    else:
        img = chart_renderer.render_wait_card(payload, candles_1h)

    # 단계별 display_type 결정
    candidate_direction = decision.get("candidate_direction") or decision.get("direction", "WAIT")
    cand_upper = str(candidate_direction).upper()

    if step_type == "EARLY":
        display_type = "EARLY"
    elif step_type == "WAIT" and cand_upper in ("LONG", "SHORT"):
        display_type = "WATCH"
    else:
        display_type = step_type

    caption = f"\U0001f4cc <b>{display_type}</b> | {symbol}"
    if display_type == "EARLY":
        caption += f" | {cand_upper} 초기 진입"
    elif display_type == "WATCH":
        caption += f" | 방향 후보: {candidate_direction}"
    # 변곡점 단계/점수
    rev_stage_cap = decision.get("reversal_stage") or ""
    rev_score_cap = decision.get("reversal_score") or 0
    if rev_stage_cap in ("WATCH", "EARLY", "PRE", "REAL"):
        caption += f" | 변곡 {rev_stage_cap} {rev_score_cap}점"
    caption += " | 자동진입 아님"

    ok = telegram.send_photo(img, caption=caption)
