...
state 종류:
  ...
  RANGE            - 박스장 (entry_filter 판단)
  SQUEEZE          - 변동성 수축 (추세 있으면 가능)
  CHAOS            - 급변동/휩쏘 (진입 금지 — hard block)
  LOW_VOLUME_TREND - 거래량 부족+추세 유지 (진입 보류, 차단 아님)
  LOW_VOLUME       - 거래량 부족 (주의, 차단 아님)
  NO_DATA          - 데이터 부족 (hard block)
"""
...
def classify_market(candles_by_tf: dict) -> dict:
    ...
    반환:
    {
        "state":         "TREND_UP/TREND_DOWN/RANGE/SQUEEZE/CHAOS/NO_DATA/LOW_VOLUME/LOW_VOLUME_TREND",
        "tradable":      True/False,
    ...
    if not c15 or len(c15) < 50:
        return _result("NO_DATA", False, False, "캔들 데이터 부족 (50봉 미만) — 진입 금지")
    ...
    # ── 거래량 부족 — 차단 금지, 추세 유지 ─────────
    # "거래량 부족은 이유이지 차단이 아니다"
    if not vol_ok:
        if trend_aligned or trend_1h in ("UP", "DOWN") or trend_15m in ("UP", "DOWN"):
            return _result("LOW_VOLUME_TREND", True, trend_aligned,
                "거래량 부족 (추세 유지 중) — 진입 보류", details)
        else:
            return _result("LOW_VOLUME", True, trend_aligned,
                "거래량 부족 (방향 약함) — 주의", details)

    # ── SQUEEZE: 변동성 수축 ───────────────────
    # 추세가 있으면 tradable=True, 방향 없으면 False
    if bb.get("squeeze") and bb.get("width", 999) < 1.5:
        sq_tradable = trend_aligned or trend_1h in ("UP", "DOWN")
        return _result("SQUEEZE", sq_tradable, trend_aligned,
    ...
    # ── 방향 미확정 → tradable=True (entry_filter가 판단)
    return _result("RANGE", True, trend_aligned,
