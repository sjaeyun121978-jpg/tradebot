...
핵심 변경 (v2):
  신호 삭제(skip) 대신 상태 변경 반환
  → blocked=True여도 카드 전송
  → direction=WAIT, block_reason 표시
  → 사용자가 "왜 안 하는지" 알 수 있음
"""
...
def check_entry(
    ...
    if direction == "WAIT":
        return _decision(False, ["방향 미확정"], direction)

    # ── 1. 시장상태 차단 ───────────────────────────
    state = market_state.get("state", "WEAK")
    if state in ("CHAOS", "WEAK"):
        reasons.append(f"시장상태 {state} — {market_state.get('reason', '')}")
    elif state == "RANGE":
        reasons.append(f"박스장 — {market_state.get('reason', '')}")
    elif state == "SQUEEZE":
        reasons.append("변동성 수축 — 방향 확정 대기")
    ...
    if score_gap < MIN_SCORE_GAP:
        reasons.append(f"점수 갭 부족 ({score_gap:.0f} < {MIN_SCORE_GAP})")
    if confidence < MIN_CONFIDENCE:
        reasons.append(f"신뢰도 부족 ({confidence:.0f}% < {MIN_CONFIDENCE}%)")
    ...
    if vol_ratio < 0.8:
        reasons.append(f"거래량 매우 부족 ({vol_ratio:.1f}배 < 0.8배)")
    ...
    return _decision(blocked, reasons, direction)


def _decision(blocked: bool, reasons: list, direction: str) -> dict:
    return {
        "trade_allowed": not blocked,
        "blocked":       blocked,
        "block_reasons": reasons,
        "block_reason":  reasons[0] if reasons else "",
        "direction":     "WAIT" if blocked else direction,
    }
